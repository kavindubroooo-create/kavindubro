...
# Hide Streamlit Menu, Footer, Deploy Button, and Header for professional clean UI
hide_menu_style = """
<style>
...
input_data = st.text_area(
    "Facebook Links / Bulk Text Input:",
    height=150,
    placeholder=(
...
def extract_numeric_id(link):
  ...
  # Fallback if ID cannot be resolved
  return link


if st.button("Extract All IDs in Bulk"):
  if input_data:
    ...
    if extracted_ids:
      ...
      st.code(bulk_output, language="text")
    else:
      st.warning("No valid Facebook links or IDs found.")
  else:
    st.warning("Please enter some links or text first.")