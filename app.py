...
# Completely Hide Streamlit Header, Toolbar (GitHub, Edit, 3-dots menu), Footer & Deploy Button
hide_menu_style = """
<style>
...
if st.button("EXTRACT NUMERIC IDS"):
    if links_input.strip():
        ...
        for idx, link in enumerate(links):
            ...
            try:
                # Request with redirect tracking
                response = requests.get(
                    link, headers=headers, allow_redirects=True, timeout=10
                )
                real_url = response.url
                html_content = response.text

                # 1. Check URL query parameters (e.g., ?id=XXXXX or profile.php?id=XXXXX)
                match_id = re.search(r"[?&]id=(\d+)", real_url)
                if match_id:
                    numeric_id = match_id.group(1)
                else:
                    # 2. Check meta tags like Open Graph url (og:url) or app links
                    ...
                    # 3. Check App links schemes (fb://profile/ID or fb://page/ID)
                    ...
                    # 4. Deep pattern matching in the full page source HTML
                    if numeric_id == "Not Found":
                        patterns = [
                            r'"user_id"\s*:\s*"(\d+)"',
                            r'"profile_id"\s*:\s*"(\d+)"',
